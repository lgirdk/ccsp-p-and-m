...
#if (defined (_XB6_PRODUCT_REQ_) || defined(_COSA_BCM_MIPS_) || defined(_HUB4_PRODUCT_REQ_)) && !defined(CONFIG_MNG)
	faultParam = NULL;
    ...
    )
{
	...
	if (factory_reset_mask & FR_ROUTER) {
		...
		if(strcmp(temp,"1") && (strcmp("Reboot Factory reset UI", value) || strcmp("Reboot Factory reset ACS", value)))
		{
			...
			if ((syscfg_set(NULL, "X_RDKCENTRAL-COM_LastRebootReason", "Reboot Factory reset CLI") != 0))
			{
				AnscTraceWarning(("syscfg_set failed\n"));
				return -1;
			}
    ...
    if (Utopia_Init(&utctx))
    {
        ...
        // Send subnet change message to ATOM so that MESH is notified.
        {
            ...
            else
            {
                ...
                CcspTraceInfo(("Sending subnet_change notification to ATOM IP \n"));
