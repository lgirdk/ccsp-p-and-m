...
// Waiting until private lan interface is ready , so that we can assign global ipv6 address and also start dhcp server.
	...
	char cmd[100];
			...
			if(pref_len < 64)
			{
			    ...
			    _get_shell_output("syscfg get IPv6subPrefix", out, sizeof(out));
			    if(!strcmp(out,"true"))
				{
                                static int first = 0;

				memset(out,0,sizeof(out));
				_get_shell_output("syscfg get IPv6_Interface", out, sizeof(out));
				pt = out;
