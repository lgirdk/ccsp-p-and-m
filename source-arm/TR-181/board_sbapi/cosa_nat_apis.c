...
#define NAT_ICMP_TIMEOUT "nat_icmp_timeout"
    ...
    )
{
    ...
    if (Utopia_RawGet(&Ctx, NULL, NAT_TCP_TIMEOUT, idStr, sizeof(idStr)))
    {
        if (idStr[0] != '\0')
        {
            pDmlNat->X_CISCO_COM_TCPTimeout = _ansc_atoi(idStr);
        }
    }
    memset(&idStr, 0, sizeof(idStr));
    if (Utopia_RawGet(&Ctx, NULL, NAT_UDP_TIMEOUT, idStr, sizeof(idStr)))
    {
        if (idStr[0] != '\0')
        {
            pDmlNat->X_CISCO_COM_UDPTimeout = _ansc_atoi(idStr);
        }
    }
    memset(&idStr, 0, sizeof(idStr));
    if (Utopia_RawGet(&Ctx, NULL, NAT_ICMP_TIMEOUT, idStr, sizeof(idStr)))
    {
        if (idStr[0] != '\0')
        {
            pDmlNat->X_CISCO_COM_ICMPTimeout = _ansc_atoi(idStr);
        }
    }
    ...
    )
{
    ...
    sprintf(idStr,"%u", pDmlNat->X_CISCO_COM_TCPTimeout);
    ...
    sprintf(idStr,"%u", pDmlNat->X_CISCO_COM_UDPTimeout);
    ...
    sprintf(idStr,"%u", pDmlNat->X_CISCO_COM_ICMPTimeout);
...
)
{
    ...
    if ( pEntry->ExternalPortEndRange == pEntry->ExternalPort && pEntry->PublicIP.Value == 0 )
    {
        ...
        snprintf(singleInfo.rule_source, sizeof(singleInfo.rule_source), (strlen(pEntry->X_RDKCENTRAL_RuleSource) != 0) ? pEntry->X_RDKCENTRAL_RuleSource : "Manual");
    ...
    else
    {
        ...
        rangeInfo.internal_port_range_size =  ((pEntry->X_RDKCENTRAL_InternalPortEndRange) - (pEntry->InternalPort));
        if( rangeInfo.internal_port_range_size < 0)
        {
        ...
        snprintf(rangeInfo.rule_source, sizeof(rangeInfo.rule_source), (strlen(pEntry->X_RDKCENTRAL_RuleSource) != 0) ? pEntry->X_RDKCENTRAL_RuleSource : "Manual");
...
{
	if((pPortMapping->ExternalPort != 0) &&
	...
	if((pPortMapping->X_RDKCENTRAL_InternalPortEndRange < pPortMapping->InternalPort) && (pPortMapping->X_RDKCENTRAL_InternalPortEndRange != 0))
	{
...
static inline void _sent_syslog_pm_sb(char *opt, UCHAR protocol, USHORT external, USHORT external_end, USHORT internal, USHORT internal_end, UCHAR ip[4], BOOLEAN active)  
{
    char extPort[30], intPort[30];
    errno_t safec_rc = -1;
    if(external_end == 0)
    {
    ...
    {
        safec_rc = sprintf_s(intPort, sizeof(intPort), "InternelPort(s) %d~%d", internal, internal_end);
        if(safec_rc < EOK)
        {
            ERR_CHK(safec_rc);
        }
...
//  lanSetting_t           lan;
    ...
    if ( rc == SUCCESS )
    {
        ...
        pNatPMapping->X_RDKCENTRAL_InternalPortEndRange = (rangeInfo.internal_port + rangeInfo.internal_port_range_size);
        ...
        ERR_CHK(safec_rc);
        safec_rc = strcpy_s(pNatPMapping->X_RDKCENTRAL_RuleSource, sizeof(pNatPMapping->X_RDKCENTRAL_RuleSource), rangeInfo.rule_source);
        ERR_CHK(safec_rc);
    ...
    if ( rc == SUCCESS )
    {
        ...
        pNatPMapping->X_RDKCENTRAL_InternalPortEndRange = pNatPMapping->InternalPort ;
        pNatPMapping->LeaseDuration = 0;
        ...
        ERR_CHK(safec_rc);
        safec_rc = strcpy_s(pNatPMapping->X_RDKCENTRAL_RuleSource, sizeof(pNatPMapping->X_RDKCENTRAL_RuleSource), singleInfo.rule_source);
        ERR_CHK(safec_rc);
        pNatPMapping->X_CISCO_COM_Origin = COSA_DML_NAT_PMAPPING_Origin_Static;
        Utopia_Free(&Ctx, 0);
        return ANSC_STATUS_SUCCESS;
...
//    ANSC_IPV4_ADDRESS             nat_lan;  
    ...
    if ( PortFwdSingleCount != 0 )
    {
        for (i = 0; i < PortFwdSingleCount; i++, ulIndex++)
        {
            ...
            pNatPMapping[ulIndex].X_RDKCENTRAL_InternalPortEndRange= pNatPMapping[ulIndex].InternalPort;
            ...
            ERR_CHK(safec_rc);
            safec_rc = strcpy_s(pNatPMapping[ulIndex].X_RDKCENTRAL_RuleSource, sizeof(pNatPMapping[ulIndex].X_RDKCENTRAL_RuleSource), singleInfo[i].rule_source);
            ERR_CHK(safec_rc);
        }
        free(singleInfo);
        singleInfo = NULL;
    ...
    if ( PortFwdRangeCount != 0 )
    {
        for (i = 0; i < PortFwdRangeCount; i++, ulIndex++)
        {
            ...
            ERR_CHK(safec_rc);
            safec_rc = strcpy_s(pNatPMapping[ulIndex].X_RDKCENTRAL_RuleSource, sizeof(pNatPMapping[ulIndex].X_RDKCENTRAL_RuleSource), rangeInfo[i].rule_source);
            ERR_CHK(safec_rc);
#ifdef _BWG_PRODUCT_REQ_
    ...
    )
{
...
#endif        
#endif        
        _sent_syslog_pm_sb("ADD", pEntry->Protocol, pEntry->ExternalPort, pEntry->ExternalPortEndRange, pEntry->InternalPort, pEntry->X_RDKCENTRAL_InternalPortEndRange, pEntry->InternalClient.Dot,pEntry->bEnabled);
    ...
    {
        _sent_syslog_pm_sb("ADD FAILED", pEntry->Protocol, pEntry->ExternalPort, pEntry->ExternalPortEndRange, pEntry->InternalPort, pEntry->X_RDKCENTRAL_InternalPortEndRange, pEntry->InternalClient.Dot,pEntry->bEnabled);
        Utopia_Free(&Ctx, 0);
        return ANSC_STATUS_FAILURE;
    }
}
    ...
    )
{
    ...
    if(rc == SUCCESS)
    {
        ...
        if(rc != SUCCESS){ 
            ...
            _sent_syslog_pm_sb("EDIT FAILED", pEntry->Protocol, pEntry->ExternalPort, pEntry->ExternalPortEndRange, pEntry->InternalPort, pEntry->X_RDKCENTRAL_InternalPortEndRange, pEntry->InternalClient.Dot,pEntry->bEnabled);
            Utopia_Free(&Ctx, 0);
            return ANSC_STATUS_FAILURE;
        }
    }
    ...
    if( ANSC_STATUS_SUCCESS == _AddPortMapping(&Ctx, pEntry))
    {
        ...
        _sent_syslog_pm_sb("ADD", pEntry->Protocol, pEntry->ExternalPort, pEntry->ExternalPortEndRange, pEntry->InternalPort, (pEntry->X_RDKCENTRAL_InternalPortEndRange-pEntry->InternalPort), pEntry->InternalClient.Dot,pEntry->bEnabled);
        return ANSC_STATUS_SUCCESS;
    }
    else
    {
        Utopia_Free(&Ctx, 1);
        _sent_syslog_pm_sb("ADD FAILED", pEntry->Protocol, pEntry->ExternalPort, pEntry->ExternalPortEndRange, pEntry->InternalPort, (pEntry->X_RDKCENTRAL_InternalPortEndRange-pEntry->InternalPort), pEntry->InternalClient.Dot,pEntry->bEnabled);
        return ANSC_STATUS_FAILURE;
    }
