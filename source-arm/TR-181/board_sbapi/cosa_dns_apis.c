    ...
    )
{
    ...
    *pulCount = dnsCount;
    ...
    for (i = 0; i < dhcpDnsCount; i++)
    {
        ...
        snprintf(server_enable, sizeof(server_enable), "dhcp_dns_server_enable_%lu", pServer[i].InstanceNumber);
    ...
    )
{
    ...
    if (pEntry->InstanceNumber <= DHCP_DNS_NUMBER * 2) //This checking is done to set the value for type DHCP separately
    {
         ...
         snprintf(server_enable, sizeof(server_enable), "dhcp_dns_server_enable_%lu", pEntry->InstanceNumber);
